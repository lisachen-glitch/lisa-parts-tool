...
app.post('/api/search', async (req, res) => {
    ...
    try {
        ...
        let prefix = "Component", mainVal = "", mat = "", tol = "", vol = "", pkg = "", pwr = "", pdesc = "";
        ...
        // 這裡保證不出現 undefined：只要變數是空的就不加入陣列
        const specsArr = [];
        if (mainVal) specsArr.push(mainVal);
        if (mat) specsArr.push(mat);
        if (tol) specsArr.push(tol);
        if (vol) specsArr.push(vol);
        if (pwr) specsArr.push(pwr);
        if (pdesc) specsArr.push(pdesc);
